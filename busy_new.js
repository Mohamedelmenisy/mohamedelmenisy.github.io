...
// ===== Language Toggle Function (CORRECTED) =====
function toggleLanguage() {
    const button = document.getElementById("lang-toggle-button");
    const appWrapper = document.querySelector(APP_SELECTOR);
    // --- Start of Correction: Get references to content divs ---
    const enContent = document.getElementById('en-content');
    const arContent = document.getElementById('ar-content');
    // --- End of Correction ---

    if (!button || !appWrapper || !enContent || !arContent) return;
    ...
    if (isArabicActive) {
        ...
        // --- Start of Correction: Show English, Hide Arabic ---
        ...
        // --- Start of Correction: Hide English, Show Arabic ---
...
function restoreLanguagePreference() {
    try {
        const preferredLang = localStorage.getItem('preferred-language') || 'ar'; // Default to Arabic
        const appWrapper = document.querySelector(APP_SELECTOR);
        const button = document.getElementById("lang-toggle-button");
        const enContent = document.getElementById('en-content');
        const arContent = document.getElementById('ar-content');
        
        if (appWrapper && button && enContent && arContent) {
            if (preferredLang === 'ar') {
                appWrapper.setAttribute('dir', 'rtl');
                button.textContent = "Switch to English";
                button.setAttribute('aria-label', 'التحويل للغة الإنجليزية');
