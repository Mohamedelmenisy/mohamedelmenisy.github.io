document.addEventListener('DOMContentLoaded', async () => {
    ...
    // بيانات افتراضية لو kbSystemData مش موجود
    const fallbackKbSystemData = {
        meta: { version: "0.1.0-fallback", lastGlobalUpdate: "2025-05-24T20:39:00Z" },
        sections: [{ id: "home", name: "Home", description: "Default section", icon: "fas fa-home", themeColor: "gray" }]
    };

    // وظيفة للانتظار حتى يتحمل kbSystemData
    async function waitForKbSystemData(maxAttempts = 5, delay = 500) {
        ...
        for (let attempt = 1; attempt <= maxAttempts; attempt++) {
            ...
            console.log(`[app.js] Attempt ${attempt}/${maxAttempts} - kbSystemData not available yet. Retrying in ${delay}ms...`);
    ...
    if (typeof protectPage === 'function') {
        ...
        console.warn('[app.js] Authentication mechanism not found. Proceeding without auth.');
    }

    const currentUser = (typeof Auth !== 'undefined' && Auth.getCurrentUser) ? Auth.getCurrentUser() : null;
    console.log('[app.js] Current user:', currentUser || 'No user found');
    ...
    if (currentUser) {
        ...
        welcomeUserName.textContent = 'Welcome, Guest!';
    }
    ...
    if (kbSystemData.meta) {
        ...
        console.warn('[app.js] kbSystemData.meta not available. Using fallback values.');
        if (kbVersionSpan) kbVersionSpan.textContent = fallbackKbSystemData.meta.version;
        if (footerKbVersionSpan) footerKbVersionSpan.textContent = fallbackKbSystemData.meta.version;
        if (lastKbUpdateSpan) lastKbUpdateSpan.textContent = new Date(fallbackKbSystemData.meta.lastGlobalUpdate).toLocaleDateString();
    }
    ...
    const initialPageContent = pageContent ? pageContent.innerHTML : '<p class="text-center text-red-500">Error: pageContent missing on load.</p>';
    ...
    function displaySectionContent(sectionId, itemIdToFocus = null, subCategoryFilter = null) {
        ...
        if (!pageContent) {
            console.error('[app.js] CRITICAL: pageContent element not found.');
            return;
        }
        ...
        if (sectionId === 'home') {
            ...
            if (currentUser && welcomeUserEl) {
                welcomeUserEl.textContent = `Welcome, ${currentUser.fullName || currentUser.email || 'User'}!`;
            }
        ...
        if (!sectionData) {
            pageContent.innerHTML = `<div class="p-6 text-center"><h2 class="text-xl font-semibold text-red-500">Section not found</h2><p>"${escapeHTML(sectionId)}" does not exist.</p></div>`;
    ...
    document.body.addEventListener('click', function(e) {
        ...
        const homeSubcatTrigger = e.target.closest('[data-subcat-filter]');
        if (homeSubcatTrigger && pageContent.querySelector('#welcomeUserName')) {
            e.preventDefault();
            const sectionId = homeSubcatTrigger.dataset.sectionTrigger;
            const subId = homeSubcatTrigger.dataset.subcatFilter;
    ...
    console.log('[app.js] All initializations complete at', new Date().toISOString());
});
