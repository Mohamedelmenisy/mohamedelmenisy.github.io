document.addEventListener('DOMContentLoaded', () => {
    ...
    (async () => {
        try {
            ...
            if (currentUser) {
                ...
                if (userNameDisplay) {
                    userNameDisplay.textContent = userDisplayName;
                    console.log('[app.js - FIX] Updated userNameDisplay to:', userDisplayName);
                }
                if (welcomeUserName) {
                    welcomeUserName.textContent = `Welcome, ${userDisplayName}!`;
                    console.log('[app.js - FIX] Updated welcomeUserName to:', `Welcome, ${userDisplayName}!`);
                }
            } else {
                console.warn('[app.js - FIX] No currentUser, skipping user name display update.');
                if (welcomeUserName) {
                    welcomeUserName.textContent = 'Welcome, Guest!';
                    console.log('[app.js - FIX] Set welcomeUserName to Guest as fallback.');
                }
            }
            ...
            if (logoutButton && typeof Auth !== 'undefined' && Auth.logout) {
                logoutButton.addEventListener('click', async (e) => {
                    ...
                    try {
                        await Auth.logout();
                        console.log('[app.js - FIX] Logout button triggered successfully.');
                    } catch (error) {
                        console.error('[app.js - FIX] Logout button error:', error);
                    }
                });
            } else {
                console.warn('[app.js - FIX] Logout button or Auth.logout not found:', {
                    logoutButton: logoutButton ? 'Found' : 'Not found',
                    AuthLogout: (typeof Auth !== 'undefined' && Auth.logout) ? 'Found' : 'Not found'
                });
            }
            ...
            function displaySectionContent(sectionId, itemIdToFocus = null, subCategoryFilter = null) {
                ...
                if (sectionId === 'home') {
                    ...
                    if (currentUser && welcomeUserEl) {
                        ...
                        console.log('[app.js - FIX] Updated welcomeUserName on home page to:', `Welcome, ${userDisplayName}!`);
                    }
            ...
            console.error('[app.js - FIX] Error during initialization:', error);
        }
    })();
    ...
    async function saveCaseToSupabase() {
        try {
            ...
            if (error) {
                console.error('[app.js] Error saving case:', error);
            ...
            console.error('[app.js] Unexpected error in saveCaseToSupabase:', error);
    ...
    async function logView(itemId, itemType = 'article') {
        try {
            ...
            if (error) {
                console.error('[app.js] Error logging view:', error);
            } else {
                console.log('[app.js] View logged successfully for item:', itemId);
            }
        } catch (error) {
            console.error('[app.js] Unexpected error in logView:', error);
        }
    }
});
