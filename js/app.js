...
document.addEventListener('DOMContentLoaded', async () => {
        ...
        if (loadingOverlay) {
            loadingOverlay.innerHTML = `
                <div class="text-center p-4">
                    ...
                    <p class="text-gray-600 dark:text-gray-400">Please ensure 'data.js' is loaded correctly and defines 'kbSystemData'.</p>
    ...
    console.log('[app.js] Using global kbSystemData:', kbSystemData);
    ...
    function handleSectionTrigger(sectionId, itemId = null, subCategoryFilter = null) {
        ...
        if (!kbSystemData || !kbSystemData.sections) {
            ...
            if (pageContent) pageContent.innerHTML = '<p class="p-4 text-red-500">Error: Knowledge base data is not available.</p>';
            return;
        }
    ...
    function displaySectionContent(sectionId, itemIdToFocus = null, subCategoryFilter = null) {
        ...
        if (!kbSystemData || !kbSystemData.sections) {
            pageContent.innerHTML = '<div class="p-6 text-center"><h2 class="text-xl font-semibold text-red-600">Error Loading Data</h2><p>The knowledge base data could not be loaded.</p></div>';
            return;
        }
        ...
        if (!sectionData) {
            pageContent.innerHTML = `<div class="p-6 text-center card-animate"><h2 class="text-2xl font-semibold text-red-500">Section Not Found</h2><p>"${escapeHTML(sectionId)}" does not exist.</p><a href="#home" data-section-trigger="home" class="mt-4 inline-block px-4 py-2 bg-indigo-600 text-white rounded hover:bg-indigo-700">Go to Home</a></div>`;
            if (currentSectionTitleEl) currentSectionTitleEl.textContent = 'Not Found';
            if (breadcrumbsContainer) breadcrumbsContainer.innerHTML = `<a href="#home" data-section-trigger="home" class="hover:underline">Home</a> <span class="mx-1">></span> Not Found`;
            return;
        }
        ...
        if (breadcrumbsContainer) {
            ...
            if (subCategoryFilter && sectionData.subCategories) {
                ...
                if (subCatData) {
                    bcHTML = `<a href="#home" data-section-trigger="home" class="hover:underline">Home</a> <span class="mx-1">></span> <a href="#${escapeHTML(sectionData.id)}" data-section-trigger="${escapeHTML(sectionData.id)}" class="hover:underline ${theme.cta}">${escapeHTML(sectionData.name)}</a> <span class="mx-1">></span> <span class="${theme.text} font-medium">${escapeHTML(subCatData.name)}</span>`;
                }
            }
        ...
        if (itemIdToFocus) {
            setTimeout(() => {
                ...
                if (targetCard) {
                    ...
                    targetCard.classList.add('ring-4', 'ring-offset-2', 'ring-indigo-500', 'focused-item');
                    setTimeout(() => targetCard.classList.remove('ring-4', 'ring-offset-2', 'ring-indigo-500', 'focused-item'), 3500);
                }
            }, 200);
        }
    }
    ...
    document.body.addEventListener('click', function(e) {
        ...
        if (ratingBtn) {
            e.preventDefault();
            alert(`Feedback: ${ratingBtn.dataset.rating} for ${ratingBtn.dataset.itemType} "${ratingBtn.dataset.itemId}". (Placeholder)`);
    ...
    function searchKb(query) { // هذه الدالة تستخدم kbSystemData المعرف عالميًا
        ...
        results.sort((a,b) => (a.title > b.title) ? 1 : ((b.title > a.title) ? -1 : 0)); // Simple sort
        return results;
    }
    ...
    if (globalSearchInput && searchResultsContainer) {
        ...
        globalSearchInput.addEventListener('input', () => {
            ...
            debounceTimeout = setTimeout(() => {
                ...
                if (results.length === 0) {
                    searchResultsContainer.innerHTML = `<div class="p-4 text-sm text-center">No results for "${escapeHTML(query)}".</div>`;
                } else {
                    let resultsHTML = results.slice(0, 10).map(result => {
                        ...
                        return `<a href="#${itemPath}" ${triggerAttrs} class="block p-3 hover:bg-gray-100 dark:hover:bg-gray-700 rounded-md transition-colors border-b dark:border-gray-600 last:border-b-0">
                            <div class="flex items-start space-x-3">
                                <div class="p-1.5 rounded-full ${theme.iconContainer} flex-shrink-0 mt-1"><i class="fas ${iconMap[result.type] || 'fa-question-circle'} text-base ${theme.icon}"></i></div>
                                <div class="flex-grow overflow-hidden">
                                    <h4 class="text-sm font-semibold truncate">${highlightText(result.title, query)}</h4>
                                    <p class="text-xs text-gray-600 dark:text-gray-400 mt-0.5">${highlightText(truncateText(result.summary, 100), query)}</p>
                                    <div class="text-xs text-gray-500 dark:text-gray-400 mt-1"><span class="font-medium ${theme.text}">${escapeHTML(result.sectionName)}</span> <span class="mx-1">•</span> <span>${result.type.replace(/_/g, ' ').replace(/\b\w/g, l => l.toUpperCase())}</span></div>
                                </div></div></a>`;
                    }).join('');
